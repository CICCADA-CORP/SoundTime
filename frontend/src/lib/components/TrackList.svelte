...
{#if menuTrackId}
  ...
  >
    ...
    <!-- Play Similar — starts embedding-based similarity radio from this track -->
    <button class="w-full text-left px-4 py-2 text-sm hover:bg-[hsl(var(--secondary))] transition flex items-center gap-3" onclick={() => {
        if (menuTrackId) {
          const trk = tracks.find((t) => t.id === menuTrackId);
          const lbl = trk ? trk.title : "Similar";
          getRadioStore().startRadio("similar", {
            seedId: menuTrackId,
            label: lbl,
          });
        }
