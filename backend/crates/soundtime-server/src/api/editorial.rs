...
//!   - `ai_max_tracks`   — Max tracks to send to AI (default: 500)
//!   - `instance_language` — Language for playlist names/descriptions (default: English)
...
pub async fn generate_editorial_playlists(
    ...
    let count = generate_editorial_inner(&state).await.map_err(|msg| {
        (
    ...
    Ok(Json(GenerateResult {
        playlists_created: count,
        message: format!("{count} editorial playlists created successfully"),
    }))
}
...
// ─── Shared types for AI generation ─────────────────────────────────
...
struct AiPlaylist {
    ...
    name: String,
    description: String,
    #[serde(alias = "tracks", alias = "songs")]
...
async fn generate_editorial_inner(state: &AppState) -> Result<usize, String> {
    ...
    // BUG-AI-3: Read configurable max tracks (default 500)
    let max_tracks = get_setting(state, "ai_max_tracks")
        .await
    ...
    // BUG-AI-3: Truncate track list if estimated tokens exceed budget
    let estimated_tokens = track_json.len() / 4;
    let (_track_list, track_json) = if estimated_tokens > 120_000 {
    ...
    // BUG-AI-5: Read instance language for prompt localization
    let instance_language = get_setting(state, "instance_language")
        .await
...
- Each playlist needs a creative name in {instance_language}
- Each playlist needs a short description in {instance_language} (1-2 sentences)
...
[
  {{
    ...
    // BUG-AI-4: Create client with timeout and single retry on failure
    ...
    let resp = match send_request().await {
        ...
        Err(first_err) => {
            ...
            send_request().await.map_err(|e| {
                ...
                "AI API request failed after retry".to_string()
            })?
        }
    };
    ...
    // BUG-AI-2: Read body as text first, then parse as JSON
    ...
    let ai_body: serde_json::Value = serde_json::from_str(&response_text).map_err(|_e| {
        ...
        tracing::error!("AI response is not valid JSON");
        tracing::debug!(preview, "AI response body preview");
        format!(
            "AI response is not valid JSON (starts with {:?})",
            &response_text[..response_text.len().min(50)]
        )
    })?;
